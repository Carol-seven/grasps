#' Plot for Object of Class "pen"
#'
#' @description
#' Generate a visualization of penalty functions produced by \code{pen()}.
...
#'
#' @param x An object of S3 class "pen".
#'
  ...
  if (key == "") {
    ggplot(df, aes(x = omega, y = value)) +
      ...
      labs(x = expression(italic(omega)),
           y = expression("Penalty Function" ~ italic(lambda) * italic(p) ~ "(" * italic(omega) * ")")) +
      theme_bw() +
